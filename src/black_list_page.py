...
def schedule_onetime(robot):
    ...
    return f"Robô agendado para execução em: {scheduled_time}", schedule

def schedule_daily(robot):
    """
    ...
    return f"""Robô agendado para execução diária a cada {execution_times}
            ...
            às {execution_time.strftime('%H:%M')}""", schedule

def schedule_weekly(robot):
    """
    ...
    return f"""Robô agendado para execução semanal a cada {execution_times}
            vezes nos dias: {', '.join(one_time_op)}""", schedule

def schedule_monthly(robot):
    """
    ...
    return f"""Robô agendado para execução mensal nos meses: {', '.join(one_time_op)}
             nos dias: {', '.join(execution_day)}""", schedule
...
def read_config_path(bot_option):
    ...
    :param bot_option: The `bot_option` parameter is the name of the robot for which you want to read
    the configuration path
    :return: a dictionary containing the values from the specified row in the "df_config" DataFrame. If
    there is no configuration for the specified robot, it returns None.
    """
...
def config_run():
    ...
    with tab1:
        ...
        if execution_op == 'Uma vez':
            msg, task = schedule_onetime(robot)

        #Se for executado diariamente
        elif execution_op == 'Diariamente':
            msg, task = schedule_daily(robot)

        #Se for semanalente
        elif execution_op == 'Semanalmente':
            msg, task= schedule_weekly(robot)

        #Se for mensalmente
        elif execution_op == 'Mensal':
            msg, task = schedule_monthly(robot)
